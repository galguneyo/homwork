...
페이지의 벤더 식별 마크는 **자체 제작한 모노그램 배지**이며 Workday · SAP의 공식 로고가 아닙니다. 상표를 임의로 재현하지 않기 위한 선택이고, 대시보드 §05에 이 사실을 명시했습니다.

공식 아트워크를 쓰려면 `dashboard/template.html`의 `LOGOS` 맵에 경로나 data URI를 넣고 다시 빌드하면 그리기 대신 이미지가 렌더링됩니다.

```js
const LOGOS = { wd:"assets/workday.svg", sf:"assets/sap.svg" };
```

발행 시 해당 파일을 Artifact의 `files`로 함께 올려야 합니다.
